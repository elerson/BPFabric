...
for key in data.keys():
  ...
  for d in data[key]:
     if(d[0] == 0):
        continue
     lb_num_map[key] = d[0]%NUM_WORKERS
     break
...
for ts, buf in pcap:
   ...
   for worker in analisys_data:
      ...
      if key in analisys_data[worker]:
         ...
         gt_ = set()
         if worker in count_current:

            for ip in count_current[worker]:
               last = 0
               if ip in count_last[worker]:
                   last = count_last[worker][ip]
               current = count_current[worker][ip]
               if abs(last-current) > PHI_:
	          gt_.add(ip)
	          
	    #for ip in count_last[worker]:
            #   current = 0
            #   if ip in count_current[worker]:
            #       current = count_current[worker][ip]
            #   last = count_last[worker][ip]
            #   if abs(last-current) > PHI_:
	    #      gt_.add(ip)
        
         count_last[worker] = count_current[worker]
         count_current[worker] = {}
         if(len(gt_) > 0):
            ...
            precision = float(len(true_positive))/(len(true_positive) + len(false_positive) + 0.001)
            accuracy = float(len(true_positive))/(len(gt_))
            recall = float(len(true_positive))/(len(true_positive) + len(false_negative) + 0.001)
   ...
   if(worker not in count_current):
      count_last[worker] = {}
      count_current[worker] = {}
      
   if ip_src not in count_current[worker]:
      count_current[worker][ip_src] = 0
   count_current[worker][ip_src] += ip.len
